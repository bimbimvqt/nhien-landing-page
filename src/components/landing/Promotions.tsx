...
import PromoGiftCard from "./PromoGiftCard";
...
export default function Promotions({ settings }: PromotionsProps) {
  ...
  const activeTasks = useMemo(() => {
    const tasks = settings?.reward_tasks || REWARD_TASKS;
    return tasks.filter((t: any) => t.active !== false) as RewardTask[];
  ...
  const loadUserEngagement = React.useCallback(
        ...
        setCompletedTaskKeys(
          (data.completed_tasks || []).filter(
            (key: any): key is RewardTaskKey =>
              activeTasks.some((task) => task.key === key),
          ),
        );
  ...
  React.useEffect(() => {
    let mounted = true;
    let claimsSubscription: any = null;
    const setupClaimsRealtime = (userId: string | undefined) => {
      ...
      const channel = supabase
        .channel(`realtime:promo_claims_${userId}`)
        .on(
          'postgres_changes',
          {
            event: '*',
    ...
    supabase.auth.getSession().then(({ data }) => {
      ...
      setupClaimsRealtime(data.session?.user?.id);
    });
    ...
    const {
      ...
      setupClaimsRealtime(currentUser?.id);
    });
  ...
  const requiredTasks = activeTasks.length;
  const onlineCompletedCount = Array.from(completedKeys).filter(
    (key) => activeTasks.some((t) => t.key === key),
  ).length;
  const canClaim = onlineCompletedCount >= requiredTasks;

  const firstClaimablePromotion = promotions.find(isPromotionClaimable);
  const claimedPromotionIds = new Set(
    claims.map((claim) => claim.promotion_id),
  );
            ...
            <div className="mt-8 rounded-[2rem] border border-sera-ink/10 bg-sera-cream p-5">
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-2xl font-black text-sera-ink">
                    {onlineCompletedCount}/{requiredTasks} nhiệm vụ
                  </p>
                  <p className="text-[10px] font-semibold text-sera-muted/80 mt-1">
                    (Hãy hoàn thành tất cả nhiệm vụ đang hiển thị để nhận quà)
                  </p>
                </div>
              ...
              <div className="mt-5 h-2 overflow-hidden rounded-full bg-sera-ink/10">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, (onlineCompletedCount / requiredTasks) * 100)}%`,
                  }}
                />
              </div>
              {!user && (
          ...
          <div className="space-y-6">
            <div className="grid gap-4 sm:grid-cols-2">
              {activeTasks.map((task) => {
                const isDone = completedKeys.has(task.key as RewardTaskKey);
                    ...
                    <div className="mb-5 flex items-start justify-between gap-3">
                      <div
                        className={cn(
                          ...
                          isDone
                            ? "bg-sera-sage text-white"
                            : "bg-sera-cream text-sera-ember",
                        )}
                      >
                        {isDone ? (
                      ...
                      <SeraBadge
                        tone={isDone ? "dark" : "warm"}
                        className="tracking-wide"
                      >
                        {isDone ? "Xong" : task.reward}
                      </SeraBadge>
                    </div>
                    <h3 className="text-lg font-black leading-tight text-sera-ink">
                      {task.title}
                    </h3>
            ...
            <div className="rounded-[2.5rem] border border-sera-ink/10 bg-sera-cream p-6 md:p-8 shadow-[0_20px_50px_-30px_rgba(39,32,28,0.3)]">
              <div className="mb-6 flex items-center justify-between gap-4">
                ...
                <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-sera-ember text-white">
              ...
              {loading ? (
                ...
                <div className="rounded-[2.5rem] bg-sera-surface p-8 sm:p-12 text-center border border-sera-ink/10 flex flex-col items-center justify-center gap-4 relative overflow-hidden group shadow-inner">
                  <div className="absolute top-0 left-0 w-24 h-24 bg-sera-ember/5 rounded-full blur-2xl pointer-events-none" />
                  <div className="w-16 h-16 rounded-2xl bg-sera-cream flex items-center justify-center text-sera-ember shadow-md shadow-sera-ember/5 mb-2 group-hover:scale-110 transition-transform duration-300">
                    <Gift className="h-8 w-8 animate-bounce" style={{ animationDuration: '3s' }} />
                  </div>
                  <div>
                    <h4 className="text-white text-lg font-black tracking-tight">Hộp quà đang được chuẩn bị!</h4>
                    <p className="mt-2 text-sm leading-relaxed text-white/50 max-w-sm mx-auto font-medium">
                      Hiện tại các chương trình ưu đãi đã kết thúc. Đội ngũ Nhiên CàFe đang chuẩn bị thêm những phần quà bất ngờ mới dành riêng cho bạn. Hãy thường xuyên quay lại kiểm tra nhé!
                    </p>
                  </div>
                </div>
              ) : (
                <div
                  className={cn(
                    ...
                    promotions.length === 1
                      ? "flex justify-center"
                      : "grid gap-6 sm:grid-cols-2 justify-center justify-items-center",
                  )}
                >
                  {promotions.map((promotion) => {
                    ...
                    return (
                      <PromoGiftCard
                        ...
                        isClaimed={claimed}
                        canClaim={canClaim}
                        canUsePromotion={canUsePromotion}
                        isBusy={busyKey === promotion.id}
                        onClaim={() => claimPromotion(promotion)}
                        onCopy={() =>
                          copyCode(claim?.code_snapshot || promotion.code)
                        }
                      />
                    );
                  })}
              ...
              <SeraLinkButton
                ...
                variant="ghost"
                size="md"
                className="mt-6 w-full border border-sera-ink/10 hover:bg-sera-ink/5"
              >
